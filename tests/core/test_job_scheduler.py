...
def mock_apscheduler_ok():
    with patch("src.core.job_scheduler.APSCHEDULER_OK", True):
        ...
        with patch("src.core.job_scheduler.AsyncIOScheduler") as mock_scheduler_cls:
...
async def test_job_scheduler_apscheduler_ok(mock_apscheduler_ok):
    ...
    with patch("src.core.job_scheduler.IntervalTrigger") as mock_interval:
    ...
    with patch("src.core.job_scheduler.CronTrigger") as mock_cron:
    ...
    future_date = datetime.now() + timedelta(days=1)
    with patch("src.core.job_scheduler.DateTrigger") as mock_date:
        scheduler.add_date("test_date", dummy_func, run_date=future_date)
